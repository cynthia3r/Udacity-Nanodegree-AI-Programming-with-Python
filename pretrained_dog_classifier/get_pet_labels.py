...
#                     
# PROGRAMMER: SAYANTANI CHAUDHURI
...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
        ...
        if in_files[idx][0] != ".":
            ...
            pet_label_lowcase = in_files[idx].lower()
            # Splits lower case string by _ to break into words
            pet_label_split_list = pet_label_lowcase.split("_")
            # Loops to check if the words in pet-label_split_list is only
            # alphabetic characters - if true append word
            # to pet_label separated by trailing space
            for item in pet_label_split_list:
                if item.isalpha():
                    pet_label += item + " "
    ...
    # Print key value pairs in the created dictionary
    for key in results_dict:
        print("Filename: {}, pet label: {}".format(key, results_dict[key]))
